...
3. 本次思路仍沿用`作业5`的单文件运行思路，没有`pom.xml`文件。
4. 针对`任务4`，`src`文件夹中有一个测试失败的文件`other_factors_failed`，在`output`中有对应的其输出失败的结果，本意想要统计分析各个用户交易大于均值和小于均值的星座、性别、城市分布情况，但是因为较为复杂处理的十分混乱，因此没有输出结果。
...
2. Reducer类：`flowReducer`类接收这些键值对，统计计算每个星期几的购买和赎回金额的总值并最后求平均值，分别写入`map_pur`和`map_red`中。在`cleanup`方法中，将`map_pur`中的数据按购入量从高到低排序，在这个map遍历中按排好的顺序最后输出。
### 运行结果
运行成功截图如下：
![task2](img/task2.png)
输出结果展示如下：
...
1. Mapper阶段：`otherMapper`类读取每一行数据，跳过第一行标题行，然后解析日期和两个利率值（日利率和年利率），并根据条件（日利率是否大于等于年利率）输出键值对。若日利率>=年利率，则输出键值对`<日期,1>`，否则输出键值对`<日期,0>`。
2. Reducer阶段：`otherReducer`类统计满足条件的天数，并在cleanup方法中输出总结信息。
### 运行结果
运行成功截图如下：
![task4](img/task4.png)
部分输出结果展示如下：
...
1. 错误处理：当前代码在解析利率值时没有进行错误处理，如果输入数据格式不正确，可能会导致`NumberFormatException`。
2. 性能优化：如果输入数据量非常大，可以考虑使用Combiner来减少Mapper和Reducer之间的数据传输量。
3. 配置管理：可以将Hadoop作业的配置参数提取到一个单独的配置文件中，以便于管理和修改。
